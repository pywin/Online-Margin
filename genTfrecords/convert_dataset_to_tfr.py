...
def main(unused_args):
    ...
    if FLAGS.dataset == 'celebext': 
        image_path = os.path.join(dir_path, 'dataset', 'celebext_data.txt')
        label_path = os.path.join(dir_path, 'dataset', 'celebext_label.txt')
        train_filename =  os.path.join('/data/face', 'dataset', 'celebext_align.tfrecords')  # tfrecords path
    ...
    for i in range(len(train_addrs)):
        if not i % 1000:
            logging.info('Train data: {}/{}'.format(i, len(train_addrs)))  
	img_path =  os.path.join('/data/face/vggface2/align/', train_addrs[i].strip())
	...
	'''	
        ...
        try:
            ...
            continue
        # get label
        label = int(train_labels[i].strip())
        rows = img.shape[0] # 218->128
        cols = img.shape[1] # 178->96
        chans = img.shape[2] # 3
        # create attr
        feature = {'height': _int64_feature(rows),
		   'width': _int64_feature(cols),
		   'channel': _int64_feature(chans),
    ...
    logging.info('convert {} to tfrecords completed: {} images!'.format(FLAGS.dataset, len(train_addrs)))
    writer.close()
    sys.stdout.flush()  
#.............................................................................
